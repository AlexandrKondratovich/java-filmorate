...
public class DirectorDbStorage implements DirectorStorage {
    ...
    public List<Director> getAllDirectors() {
        ...
        log.info("Найдено {} режиссер", directorList.size());
        return directorList;
    }
    ...
    public Optional<Director> getDirectorById(int id) {
        ...
        if (!directorList.isEmpty()) {
            log.info("Найден режиссер с ID: {} и названием {} ", directorList.get(0).getId(), directorList.get(0).getName());
            return Optional.of(directorList.get(0));
        } else {
            log.info("Режиссера c идентификатором {} не найдена", id);
